...
public class DrugInformationController {
    ...
    @ResponseBody
    public String uploadFilesToExcel(@RequestParam(value = "fileName")MultipartFile file, HttpServletResponse response, HttpServletRequest request){
        String msg = "导入失败";
        ...
        int i = drugInformationService.addDrugInformation(drugItems);
        drugMessage.setDrugItemsId(drugItems.getDiId());
        int j = drugInformationService.addDrugMessageInfo(drugMessage);
        if (i>0 && j>0){
            msg = "导入成功";
        }else {
            msg = "导入失败";
    ...
    public void downloadTemplate(HttpServletResponse response){
        try {
            drugInformationService.downloadTemplate(response);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
